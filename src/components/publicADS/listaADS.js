...
export default class AdsList extends Component {
  ...
  adCaller = async () => {
    this.setState({
      responseState: await addCall(
        createQueryString(
          this.state.formName,
          this.state.formSellOrBuy,
          this.state.formTags,
          this.state.formPriceMin,
          this.state.formPriceMax
        )
      ),
    });
    this.evaluator();
  ...
  render() {
    ...
    return (
      <>
        <form className="bg-dark" onSubmit={this.onSubmitController}>
          ...
          <div className="form-group d-flex">
            <input
              ...
              placeholder="Minimo"
              value={formPriceMin}
              onChange={this.priceMinController}
            ></input>
            <input
              className="form-control m-2"
              type="number"
              placeholder="Maximo"
              value={formPriceMax}
              onChange={this.priceMaxController}
            ></input>
          </div>
          <input
        ...
        <RenderOrNot
          callDo={this.state.childrenToRender}
          data={this.state.responseState}
        />
